...
public class AppointmentDAO {
    ...
    private static final String GET_ACTIVE_APPOINTMENT = "SELECT a.id, user_id, appointment_date, kind_of_pet, pet_name, s.status " +
            "FROM appointment a join statuses s on s.id = a.status_id WHERE s.status = 'ACTIVE' and user_id = ?";

    private static final String GET_APPOINTMENT_BY_ID = "SELECT a.id, user_id, appointment_date, kind_of_pet, pet_name, s.status " +
            "FROM appointment a join statuses s on s.id = a.status_id WHERE a.id = ?";

    private static final String GET_ALL_BY_USER_ID = "SELECT a.id, user_id, appointment_date, kind_of_pet, pet_name, s.status " +
            "FROM appointment a join statuses s on s.id = a.status_id WHERE user_id = ?";

    private static final String INSERT_APPOINTMENT = "INSERT INTO appointment(user_id, appointment_date, kind_of_pet, pet_name) " +
            " VALUES (?, ?, ?, ?)";

    private static final String UPDATE_APPOINTMENT = "UPDATE public.appointment " +
            " SET user_id=?, appointment_date=?, kind_of_pet=?, pet_name=?, status_id= (select id from statuses where status =?) " +
            " WHERE id=?;";
